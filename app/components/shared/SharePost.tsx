...
export default function SharePost({ title, slug, description }: props) {
  const blog = encodeURIComponent('https://atayev.uz/blog/');
  const options = [
    {
      icon: BiLogoTwitter,
      name: "Twitter",
    ...
    },
    {
      icon: BiLogoWhatsapp,
      name: 'WhatsApp',
      shareUrl: `https://api.whatsapp.com/send?text=${encodeURIComponent('Read this amazing article by Mansurbek Atayev')}.%0A%0A${title}%0A%0A${blog}${slug}`
    },
  ];
